...
static void bcm63xx_external_irq_mask(unsigned int irq)
{
	...
	irq -= bcm63xx_external_irq_base();
	reg = bcm_perf_readl(PERF_EXTIRQ_CFG_REG);
	if (BCMCPU_IS_6358())
		reg &= ~EXTIRQ_6358_CFG_MASK(irq);
	else
					 ...
					 unsigned int flow_type)
{
	...
	case IRQ_TYPE_EDGE_BOTH:
		if (BCMCPU_IS_6358()) {
			reg &= ~EXTIRQ_6358_CFG_LEVELSENSE(irq);
			reg |= EXTIRQ_6358_CFG_BOTHEDGE(irq);
		} else {
...
void __init arch_init_irq(void)
{
	...
	for (i = bcm63xx_external_irq_base(); i < bcm63xx_external_irq_base() + 4; ++i)
