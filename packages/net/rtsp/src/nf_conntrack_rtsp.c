				...
				if (prtspexp->loport != 0
					...
					if (ptran[off] == '-') {
						...
						if ((prtspexp->
						    ...
						    prtspexp->loport + 1) {
							pr_debug
							    ("incorrect range: %hu-%hu, correcting\n",
							     prtspexp->loport,
							     prtspexp->hiport);
							prtspexp->loport &=
							    0xfffe;
...
static inline int help_out(struct sk_buff *skb, unsigned char *rb_ptr,
			   unsigned int datalen, struct nf_conn *ct,
			   enum ip_conntrack_info ctinfo)
{
	...
	while (dataoff < datalen) {
		...
		off = 0;
		if (translen) {
			rtsp_parse_transport(pdata + transoff, translen,
					     &expinfo);
		}
		...
		struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
	...
	if (tcp_v4_check(tcph, tcplen, iph->saddr, iph->daddr,
		...
		pr_debug("bad csum: %p %u %u.%u.%u.%u %u.%u.%u.%u\n",
