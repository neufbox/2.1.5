		 ...
		 struct sk_buff *skb, uint tranoff, uint tranlen)
{
	...
	case pb_range:
		...
		if (loport != 0) {
			rbuf1len = sprintf(rbuf1, "%hu", loport);
			rbufalen =
			    sprintf(rbufa, "%hu-%hu", loport, loport + 1);
		}
		break;
	case pb_discon:
		for (loport = prtspexp->loport; loport != 0; loport++) {	/* XXX: improper wrap? */
		...
		if (loport != 0 && hiport != 0) {
			...
			if (hiport == loport + 1) {
				rbufalen =
				    sprintf(rbufa, "%hu-%hu", loport, hiport);
			} else {
				rbufalen =
				    sprintf(rbufa, "%hu/%hu", loport, hiport);
			}
	...
	while (off < tranlen) {
		...
		pparamend =
		    (pparamend == NULL) ? ptran + tranlen : pparamend + 1;
		...
		while (off < nextparamoff) {
			...
			pfieldend =
			    memchr(ptran + off, ';', nextparamoff - off);
			nextfieldoff =
			    (pfieldend ==
			     NULL) ? nextparamoff : pfieldend - ptran + 1;

			if (dstact != DSTACT_NONE
			    && strncmp(ptran + off, "destination=", 12) == 0) {
				if (strncmp
				    (ptran + off + 12, szextaddr,
				     extaddrlen) == 0) {
					is_stun = 1;
				}
				if (dstact == DSTACT_STRIP
				    || (dstact == DSTACT_AUTO && !is_stun)) {
					diff = nextfieldoff - off;
					if (!nf_nat_mangle_tcp_packet
					    (&skb, ct, ctinfo, off, diff, NULL,
					     0)) {
	 ...
	 struct ip_ct_rtsp_expect *prtspexp, struct nf_conntrack_expect *exp)
{
	...
	while (nf_mime_nextline
		...
		if (nf_strncasecmp(ptcp + lineoff, "Transport:", 10) == 0) {
			...
			if (!rtsp_mangle_tran
			    (ctinfo, exp, prtspexp, skb, lineoff, linelen)) {
			...
			pr_debug("rep: len=%u, %.*s", linelen, (int)linelen,
				 ptcp + lineoff);
		}
	}
...
static void expected(struct nf_conn *ct, struct nf_conntrack_expect *exp)
{
	...
	pr_debug
	    ("newsrcip=%u.%u.%u.%u, newdstip=%u.%u.%u.%u, newip=%u.%u.%u.%u\n",
